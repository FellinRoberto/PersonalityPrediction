...
int main(int argc, char* argv[]) {
	...
	//printf("sto per chiamare read_documents_con kernel_parm->first_kernel=%ld\n", kernel_parm.first_kernel);
	//printf("\nMax docs: %ld, approximated number of feature occurences %ld, maximal length of a line %ld\n\n",max_docs,max_words_doc,ll);
	read_documents(docfile, docs, target, max_words_doc, ll, &totwords,
			&totdoc, &kernel_parm);
	if (totwords == 0)
		totwords = learn_parm.svm_maxqpsize; //for structural kernels
	...
	if (kernel_parm.kernel_type == LINEAR) { /* don't need the cache */
		if (learn_parm.type == CLASSIFICATION) {
			svm_learn_classification(docs, target, totdoc, totwords,
		...
		if (learn_parm.type == CLASSIFICATION) {
			...
			printf(
					"Learning rankings is not implemented for non-linear kernels in this version!\n");
			exit(1);
		} else if (learn_parm.type == PERCEPTRON) {
			perceptron_learn_classification(docs, target, totdoc, totwords,
					&learn_parm, &kernel_parm, &kernel_cache, &model, modelfile);
		} else if (learn_parm.type == PERCEPTRON_BATCH) {
			batch_perceptron_learn_classification(docs, target, totdoc,
					totwords, &learn_parm, &kernel_parm, kernel_cache_size,
					&model);
		}
	...
	/*if (isSimilarityLoaded() == 1)
	 save_cache("tmp.txt");
	 */

	print_time_elapsed_tree_kernel();
...
void read_input_parameters(int argc, char *argv[], char *docfile,
		char *modelfile, long *verbosity, long *kernel_cache_size,
	...
	strcpy(kernel_parm->vector_kernel_params,""); // Default no params


	kernel_parm->similarity_threshold = 1.0;
	kernel_parm->terminal_factor = 1.0;

	kernel_parm->sim_cache_size = 1000000;
	kernel_parm->sim_cache_max_size = 10000000;
	kernel_parm->sim_cache_refresh_factor = 0.5;

	strcpy(kernel_parm->words_matrix_path,"empty");
	strcpy(kernel_parm->similarity_cache,"empty");
	...
	for (i = 1; (i < argc) && ((argv[i])[0] == '-'); i++) {
		...
		case 'L':
			i++;
			kernel_parm->lambda = atof(argv[i]);
			break;
		case 'T':
			i++;
			kernel_parm->tree_constant = atof(argv[i]);
			break;
		case 'C':
			i++;
			kernel_parm->combination_type = *argv[i];
			break;
		case 'F':
			i++;
			kernel_parm->first_kernel = atoi(argv[i]);
			break;
		case 'S':
			i++;
			kernel_parm->second_kernel = atoi(argv[i]);
			break;
		case 'V':
			i++;
			kernel_parm->vectorial_approach_standard_kernel = *argv[i];
			break;
		case 'W':
			i++;
			kernel_parm->vectorial_approach_tree_kernel = *argv[i];
			break;
		case 'M':
			i++;
			kernel_parm->mu = atof(argv[i]);
			break;
		case 'N':
			i++;
			kernel_parm->normalization = atoi(argv[i]);
			break;
			//    case 'U': i++; kernel_parm->tree_kernel_params=atoi(argv[i]); break; // user defined parameters
		case 'G':
			i++;
			strcpy(kernel_parm->vector_kernel_params,argv[i]);
			break; // user defined parameters (vector)
		case 'P':
			i++;
			strcpy(kernel_parm->words_matrix_path,argv[i]);
			break; //to use with PT SMOOTHING KERNELS
		case 'Z':
			i++;
			strcpy(kernel_parm->similarity_cache,argv[i]);
			break; //to use with PT SMOOTHING KERNELS
		case 'A':
			i++;
			kernel_parm->sim_cache_size = atoi(argv[i]);
			break; //to use with PT SMOOTHING KERNELS
		case 'B':
			i++;
			kernel_parm->sim_cache_max_size = atoi(argv[i]);
			break; //to use with PT SMOOTHING KERNELS
		case 'D':
			i++;
			kernel_parm->sim_cache_refresh_factor=atof(argv[i]);
			break; //to use with PT SMOOTHING KERNELS
		case 'H':
			i++;
			kernel_parm->similarity_threshold = atof(argv[i]);
			break; //to use with PT SMOOTHING KERNELS
		case 'X':
			i++;
			kernel_parm->terminal_factor = atof(argv[i]);
			break;//to use with PT SMOOTHING KERNELS
		default:
	...
	if (strcmp(kernel_parm->vector_kernel_params, "") != 0) {
		PARAM_VECT = 1;
		//  kernel_parm->tree_kernel_params=PARAM_VECT;

		printf("Inserted parameter file = %s\n",
				kernel_parm->vector_kernel_params);
	}
	//PARAM_VECT=kernel_parm->tree_kernel_params;
	//printf("PARAM_VECT impostato a %hd\n", kernel_parm->tree_kernel_params);
	// CONFIG_VECT=kernel_parm->vector_kernel_params;
	//printf("kernel_parm->tree_kernel_params=%d , kernel_parm->vector_kernel_params=%d\n", kernel_parm->tree_kernel_params, kernel_parm->vector_kernel_params);
	// if(PARAM_VECT == 1) read_input_tree_kernel_param(tree_kernel_params); // if there is the file tree_kernel.param load paramters

	if (PARAM_VECT == 1)
		read_input_vector_kernel_param(kernel_parm); // if there is the file param_vec.ini load paramters
	...
	if (kernel_parm->similarity_threshold < 0
			|| kernel_parm->similarity_threshold > 1.0) {
		printf("\nThe parameter similarity threshold must be in [0, 1]");
		wait_any_key();
		print_help();
		exit(0);
	}
...
void print_help() {
	...
	printf("                        6: tree kernel\n");
	printf("                        7: multi kernel\n");
	printf(
	...
	printf(
			"         -F [0,1,2,3,4,5]-> 0 = ST kernel, 1 = SST kernel, 2 = SST-bow, 3 = PT kernel, 4 = PT kernel no leaves, 5 = PT kernel with smoothing (default 1)\n");
	printf(
			"         -M float    -> Mu decay factor for PT kernel (default 0.4)\n");
	printf(
	...
	printf("         -u string   -> parameter of user defined kernel\n");
	printf("Option for Partial Tree With Smoothing:\n");
	printf(
			"         -P string  -> matrix path to be used in partial tree kernel with smoothing\n");
	printf(
			"         -Z string  -> the similarity cache path to be used in partial tree kernel with smoothing\n");
	printf("         -A float    -> Similarity cache size\n");
	printf("         -B float    -> Similarity cache max size\n");
	printf("         -D float    -> Similarity cache refresh factor\n");
	printf(
			"         -H float    -> the similarity threshold between terminal nodes to be used in tree kernel with smoothing\n");
	printf(
			"         -X float-> the lexical factor to be used in tree kernel with smoothing (default 1)\n");
