...
# include "svm_learn.h"
//#include <sys/resource.h>  Commentato da Crescenzo altrimenti non compilava
...
double estimate_r_delta(DOC *docs, long int totdoc, KERNEL_PARM *kernel_parm)
{
  ...
  for(i=0;i<totdoc;i++) {
    xlen=sqrt(kernel(kernel_parm,&(docs[i]),&(docs[i]))
	      ...
	      +kernel(kernel_parm,&nulldoc,&nulldoc));
	  /*xlen=kernel(kernel_parm,&(docs[i]),&(docs[i]));
	  printf("1\n");
	  fflush(stdout);
	  xlen-=2*kernel(kernel_parm,&(docs[i]),&nulldoc);
	  printf("2\n");
	  fflush(stdout);
	  xlen+=kernel(kernel_parm,&nulldoc,&nulldoc);
	  printf("3\n");
	  fflush(stdout);
	  xlen=sqrt(xlen);*/
    if(xlen>maxxlen) {
  ...
  //printf("FINE FUNZIONE estimate_r_delta \n");
  fflush(stdout);
  return(maxxlen);
}
...
//  strcpy(nullword.sName="NULL");
 // nullword.iNoOfChildren=0;
  ...
  avgxlen=0;
//  printf("totdoc %d\n",totdoc);
  //printf("NELLA FUNZIONE estimate_r_delta_average con totdoc pari a %ld\n", totdoc);
  for(i=0;i<totdoc;i++) {
...
//		  +kernel(kernel_parm,&nulldoc,&nulldoc));
  ...
  //printf("FINE FUNZIONE estimate_r_delta_average \n");
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  fprintf(modelfl,"%s # kernel parameter -G \n", model->kernel_parm.vector_kernel_params);

  fprintf(modelfl,"%f # similarity threshold -H \n", model->kernel_parm.similarity_threshold);
  fprintf(modelfl,"%f # terminal factor -X \n", model->kernel_parm.terminal_factor);
  fprintf(modelfl,"%s # matrix path -P \n", model->kernel_parm.words_matrix_path);
  fprintf(modelfl,"%s # cache path -Z \n", model->kernel_parm.similarity_cache);
  fprintf(modelfl,"%i # cache size -A \n", model->kernel_parm.sim_cache_size);
  fprintf(modelfl,"%i # cache max size -B \n", model->kernel_parm.sim_cache_max_size);
  fprintf(modelfl,"%f # refresh factor -D \n", model->kernel_parm.sim_cache_refresh_factor);
