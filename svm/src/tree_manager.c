 ...
 //  if(T_start!=NULL)printf("\n\n<%s>\n",T_start);fflush(stdout);
  ...
  while(T_start!=NULL && !ET){                      
      ...
      *T_end = 0; // isolate the tree string
	  ...
	  //CODICE CAMBIATO, prima era così:  if(PARAM_VECT == 1 && tree_kernel_params[tree_index].kernel_type==END_OF_TREE_KERNELS){
...
//		  CONFIG_VECT=0;//aggiunto
		}

      TKG_temp = TKGENERALITY; // save the command line TKGENERALITY
      //CODICE CAMBIATO, prima era così: if(PARAM_VECT==1)TKGENERALITY=tree_kernel_params[tree_index].TKGENERALITY;
...
int counter=0;
    ...
    if(tree->root==NULL){ 
     ...
     //printf("STO PER CHIAMARE fillTheNodeList con TKGENERALITY=%hd\n", TKGENERALITY);
