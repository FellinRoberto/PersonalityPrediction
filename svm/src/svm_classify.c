...
int main (int argc, char* argv[])
{
  ...
  strcpy(model.kernel_parm.words_matrix_path, dictionary);
...
/*        0.01 secs, the timer was underflowing.                       */
  ...
  if((!no_accuracy) && (verbosity>=1)) {
    ...
    printf("Precision/recall on test set: %.2f%%/%.2f%%\n",P=(float)(res_a)*100.0/(res_a+res_b),R=(float)(res_a)*100.0/(res_a+res_c));
...
void print_help(void)
{
  ...
  printf("         -H float   -> simarity threshold for PT kernel with smoothing \n");
  printf("         -X float   -> lexical factor for PT kernel with smoothing (default 1)\n\n");
}
